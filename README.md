...
A React, TypeScript, and Vite teaching simulator with two validated silicon-device laboratories:
...
Both laboratories use a result-first engineering dashboard: sweep controls stay
beside the terminal characteristics, while internal fields and numerical
diagnostics remain available as optional detail. An initial condition is never
...
On desktop viewports, controls stay in a compact left rail; on smaller screens
they collapse above the results. Plots use a high-density canvas backing store
so curves and labels remain sharp on HiDPI displays.
...
A recent Node.js version and pnpm are required:
...
Open the URL printed by Vite. The root page contains the PN diode and
`/bjt.html` contains the NPN transistor. To verify both numerical cores and
produce the GitHub Pages artifact:
...
The React UI and worker boundaries are TypeScript. The validated numerical
kernels remain in JavaScript and C/WebAssembly; `allowJs` keeps that migration
boundary explicit without changing the equations. The compiled NPN WASM asset
is committed. Developers with WASI SDK or compatible Clang can rebuild it with:
